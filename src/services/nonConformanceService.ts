...
// The nc_number is generated by a database trigger, so we don't include it in the input
export async function createNonConformance(nonConformance: {
  ...
  try {
    ...
    // This is necessary because Supabase's database has a trigger that generates the nc_number
    const { data, error } = await supabase
      .from('non_conformances')
      .insert(nonConformance as any)
      .select()
      .single();
